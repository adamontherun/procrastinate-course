...
CHAPTERS = [
    (
        1,
        "Why a task queue — and why Postgres",
        "What background jobs actually are, and how a database becomes a broker with SELECT FOR UPDATE and LISTEN/NOTIFY.",
    ),
    (
        2,
        "Setup: your first job, end to end",
        "Install, connect, apply the schema, defer one job, and watch a worker pick it up.",
    ),
    (
        3,
        "Defining and deferring tasks",
        "Task anatomy, configure(), batch deferring, JSON argument limits, and blueprints for larger codebases.",
    ),
    (
        4,
        "Workers up close",
        "Queues, concurrency, how jobs are fetched the instant they're deferred — and the polling that backs it up.",
    ),
    (
        5,
        "Retries and failure",
        "The job lifecycle, built-in retry strategies, and writing a custom one that changes queue or priority on retry.",
    ),
    (
        6,
        "Scheduling: later and every hour",
        "One-off future jobs with schedule_at/schedule_in, and periodic tasks with cron — including who defers them when you run five workers.",
    ),
    (
        7,
        "Locks, queueing locks, priorities",
        "Serialize jobs that touch the same resource, stop duplicate jobs from piling up, and decide who goes first.",
    ),
    (
        8,
        "Context, cancellation, middleware",
        "Reach into the running job with JobContext, abort jobs that are already executing, and wrap every task with middleware.",
    ),
    (
        9,
        "Sync and async together",
        "Sync connectors, deferring from synchronous code, and the RuntimeError waiting for you at the boundary.",
    ),
    (
        10,
        "Procrastinate in FastAPI",
        "Defer jobs from endpoints, embed a worker in the lifespan — and why production usually wants a separate worker process.",
    ),
    (
        11,
        "Testing your tasks",
        "InMemoryConnector, asserting on deferred jobs, and running a real worker inside a test when it's worth it.",
    ),
    (
        12,
        "Operating in production",
        "Schema migrations without downtime, monitoring, graceful shutdown, stalled-job recovery, and keeping the jobs table small.",
    ),
    (
        13,
        "Deploying workers on AWS ECS",
        "Containerize the worker, map SIGTERM to procrastinate's graceful shutdown, and size stopTimeout so jobs survive deploys.",
    ),
]
